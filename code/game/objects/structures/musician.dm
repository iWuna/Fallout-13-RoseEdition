...
//Портировал с хиппистейшона февраля 2021 года
// Трейты музыканта и хорошей музыки были закомменчены
// NO_TK были убраны так как это видимо трейты которых нету
#define MUSICIAN_HEARCHECK_MINDELAY 4
#define MUSIC_MAXLINES 300
#define MUSIC_MAXLINECHARS 50
...
/datum/song/proc/playnote(mob/user, note, acc as text, oct)
	...
	/*var/sound/music_played = sound(soundfile) //Пока не портируете трейты с хиппистешона - не разкомменчивать
	for(var/i in hearing_mobs)
		var/mob/M = i
...
/datum/song/proc/shouldStopPlaying(mob/user)
	if(instrumentObj)
		if(!user.canUseTopic(instrumentObj, BE_CLOSE, FALSE ))
			return TRUE
		return !instrumentObj.anchored		// add special cases to stop in subclasses
	else
		return TRUE
...
/datum/song/Topic(href, href_list)
	if(!usr.canUseTopic(instrumentObj, BE_CLOSE, FALSE ))
		usr << browse(null, "window=instrument")
		usr.unset_machine()
		return
	...
	if(href_list["repeat"]) //Changing this from a toggle to a number of repeats to avoid infinite loops.
		...
		var/newline = html_encode(input("Enter your line: ", instrumentObj.name) as text|null)
		if(!newline || !usr.canUseTopic(instrumentObj, BE_CLOSE, FALSE ))
			return
		if(lines.len > MUSIC_MAXLINES)
			return
