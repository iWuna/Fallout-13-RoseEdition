...
/obj/item/flashlight/flare/emergency
	...
	var/obj/landmark/vertibird/flare/landmark = null
	var/static/count = 1
...
/obj/item/flashlight/flare/emergency/turn_off()
	...
	anchored = FALSE
	vertibirdLandZone -= landmark
	landmark.Destroy()

/obj/item/flashlight/flare/emergency/attack_hand(mob/user)
	. = ..()
	if(anchored && on)
		user.visible_message("<span class='notice'>[user] begins to put down the [src]!</span>")
		if(do_after(user, 40, TRUE, src))
			user.visible_message("<span class='notice'>[user] puts down the [src]!</span>")
			turn_off()
